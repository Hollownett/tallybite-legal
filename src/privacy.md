...
<!--
  ...
  Load-bearing accuracy notes (2026-07-14), each pinned by a test in privacyPolicy.test.ts:
   ...
   - A label scan sends the label photo and transcribes printed nutrition. Advanced Describe sends
     typed or dictated text, never audio or a photo. When Advanced is selected, finishing voice input
     sends the complete text displayed in the field after consent; nothing is sent while recognition
     is active, and Lite stays local. The speech-to-text itself is done by the phone's own configured
     speech service, not by Gramwise.
   - Advanced AI requires the paid Pro subscription; there is no free Advanced-AI allowance and NO
     trial (owner ruling 2026-09-06: no trial offer is live in Play Console, so ADR-0048's "3-day
     trial" product claim is withdrawn — its purchase-token mechanism stands). Subscribers
     authenticate with the Google Play purchase token, which the service verifies with Google. If
     Google Play ever surfaces an introductory offer, Play alone decides eligibility. Never write a
     trial length into this policy. Allowance values come from shared code constants and must match
     both the Worker and paywall.
...
Gramwise has no account or login. Your food diary, profile, weight history, Health Connect data, and attached meal-photo library stay on your phone unless you deliberately share a backup or choose an Advanced AI action described below. The core tracker works without a network for every user: your diary, the bundled food search, describing a meal in words, your saved foods, and entering food by hand. Gramwise runs one recognition service: an Advanced meal scan sends the whole meal photo and any optional meal hint you type or dictate for that scan, a label scan sends the label photo, and Advanced Describe sends the words you typed or dictated (never the audio). The service does not retain those submitted photos, hints, descriptions, or a diary. It retains shared food-and-nutrition facts and pseudonymous operational records for quotas, subscription-verdict caching, and replay protection. Scanning a photo — a meal or a nutrition label — is Advanced AI: there is no on-device recognition, so a scan always sends the photo and cannot run without a connection. Advanced AI requires the paid Pro subscription; there is no free Advanced-AI allowance and no trial. The free tier keeps the diary, the offline search, Describe, saved foods, manual entry, and the barcode scanner, but not a photo scan. Billing shows only the offers Google Play returns for the current account, and any introductory offer would be Google Play's to decide. Subscribers use the Play purchase token, which the recognition service verifies with Google. Gramwise does not send a Play Integrity token or hashed device identifier to decide offer eligibility. Only after Billing says the user is Free can Google's consent and AdMob SDKs start; Pro subscribers do not initialise the advertising SDK. Questions: hollownett@gmail.com.
...
Photos are not uploaded, with one exception that you control — but when that exception applies, **the whole photo goes.** If you turn on **Advanced AI recognition** (Settings → AI & recognition) — nothing is sent until you turn it on, and a photo scan also needs the Pro subscription described below — and tap Scan, **your whole meal photo is sent**, uncropped, together with any optional short meal hint you typed or dictated on the approval screen, so the AI can find the foods in it for you. You can dictate that hint with the mic beside it: your phone's own speech service turns your voice into text, exactly as described for Describe below, and Gramwise never receives, stores, or logs the audio — only the words shown in the field are sent, and only when you tap Scan. It sees whatever you photographed: the food, the plate, the table, and anything else that happened to be in frame. Point the camera at your meal, not at anything you would rather not send.
...
Cloud Advanced AI needs a **Pro subscription**; there is no free Advanced-AI allowance and no trial. Sent with each Advanced request is your **Google Play purchase token**, which the service verifies with Google Play. The same token proves an active paid subscription. Gramwise does not send a Play Integrity token or hashed Android ID for offer eligibility, and it does not maintain a device trial ledger.
